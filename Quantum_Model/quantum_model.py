...
def variational_quantum_circuit(inputs, weights):
    ...
    def circuit(inputs, weights):
        ...
        # Circuit 3 - Control
        for i in range(4):
            qml.Rot(inputs[i][0], weights[i][1], inputs[i][2], wires=i)
            qml.CNOT(wires=[0, 1])
            qml.CNOT(wires=[1, 2])
        ...
        for i in range(4):
            ...
            qml.Rot(inputs[i][0], inputs[i][1], weights[i][2], wires=i)
            qml.CNOT(wires=[0, 1])
            qml.CNOT(wires=[1, 2])
            qml.CNOT(wires=[2, 3])
...
def variational_quantum_circuit(inputs, weights):
    ...
    @qml.qnode(dev)
    def circuit(inputs, weights):
        # Circuit 1 - Image classification
        for i in range(4):
            qml.RX(weights[i][0], wires=i)
        ...
        # Circuit 2 - Decision-making based on classification
        for i in range(4):
            qml.RX(inputs[i][0], wires=i)
            qml.RY(inputs[i][0], wires=i)
        ...
        # Circuit 3 - Additional functionality for self-driving car
        for i in range(4):
            qml.RY(weights[i][0], wires=i)
            qml.RX(weights[i][0], wires=i)
